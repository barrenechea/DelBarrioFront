<<template>
  <div class="container">
    <div class="bs-component">
      ...
      <table class="table table-responsive">
        ...
        <tbody>
          <tr v-for="reason in reasons">
            ...
            <td>{{reason.NOMB_MOTIVO_DENUNCIA}}</td>
            <td>
              <a class="btn btn-danger" v-on:click="setState(reason.IDEN_MOTIVO_DENUNCIA, false)" v-show='reason.FLAG_VIGENTE'>
                <i class="fa fa-times" title="Deshabilitar"></i>
              </a>
              <a class="btn btn-success" v-on:click="setState(reason.IDEN_MOTIVO_DENUNCIA, true)" v-show='!reason.FLAG_VIGENTE'>
      ...
      <a class="btn btn-success" v-bind:href="'/administracion/razon-denuncia/nuevo'">Agregar</a>
    </div>
  </div>
</div>
</template>
